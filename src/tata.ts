...
type Vertical = "t" | "m" | "b"
type Horizontal = "r" | "m" | "l"
type Position = `${Vertical}${Horizontal}`

type Options = {
    type: 'text' | 'log' | 'info' | 'warn' | 'error' | 'success' | 'ask',
    position: Position, animate: "fade" | "slide",
    duration: number,
    progress: boolean, holding: boolean,
    closeBtn: boolean, onClick?: Function, onClose?: Function
}

type TataRecords = { title: string, text: string, opts: Options, id: string, element: HTMLElement }
...
function addIconsLink(href: string) {
...
function type2Icon(type = 'text'): string {
...
function clickTaTa(event: Event) {
    const target = event.target as HTMLElement
    if (target.classList.contains('tata-close')) return
    this.opts.onClick.call(this)
}

async function closeTaTa(event: Event) {
    const target = event.target as HTMLElement
    if (!target.classList.contains('tata-close')) return
    const element = target.parentNode as HTMLElement;
    const ta: TataRecords = tataDatas.get(element)

    element.classList.add(mapAnimateOut(ta.opts.animate, ta.opts.position))

    const onClose = ta.opts.onClose;
    if (typeof onClose === 'function') onClose.call(ta)

    await scheduleRemoveTata(ta)
}

document.addEventListener('click', closeTaTa, false)
const tatas: { byPosition: { [key in Position]?: TataRecords }, byId: { [key: string]: TataRecords } } =
    {byPosition: {}, byId: {}}
const tataDatas = new WeakMap<HTMLElement, TataRecords>()

function recordTata(record: TataRecords) {
...
function removeTata(record: TataRecords) {
    if (tatas.byPosition[record.opts.position] === record) delete tatas.byPosition[record.opts.position]
    delete tatas.byId[record.id]
    record.element.remove()
}

async function scheduleRemoveTata(record: TataRecords) {
    await sleep(800);
    removeTata(record)
}

function htmlToElement(htmlStr: string): HTMLElement {
    const container = document.createElement('div');
    container.innerHTML = htmlStr;
    return container.firstElementChild as HTMLElement;
}

async function render(title: string, text: string, opts: Options) {
    const id = genId()
    ...
    if (samePosition) scheduleRemoveTata(samePosition).then()
    const ta: TataRecords = {title, text, opts, id, element}
    ...
    if (opts.holding) return;
    if (opts.progress) {
        const progress = element.querySelector('div.tata-progress') as HTMLDivElement
